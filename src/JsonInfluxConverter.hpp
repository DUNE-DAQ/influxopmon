...
namespace dunedaq {
ERS_DECLARE_ISSUE(influxopmon, IncorrectJSON, "JSON input incorrect" << Warning, ((std::string)Warning))

ERS_DECLARE_ISSUE(influxopmon, error_JSON, "JSON input error" << Error, ((std::string)Error))
...
public:
   ...
   */
  std::vector<std::string> get_inserts_vector() { return m_inserts_vector; }
      ...
      if (item.value().begin()->is_structured()) {
        ...
        for (auto& last_item : item.value().items()) {
          ...
          if (last_item.value().type() == json::value_t::string) {
            if (last_item.value().dump()[0] != '"') {
              "\"" + last_item.value().dump();
            }
            if (last_item.value().dump()[last_item.value().dump().size() - 1] != '"') {
              last_item.value().dump() + "\"";
            }
